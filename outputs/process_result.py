...
def process_result_file(filename):
    ...
    for expr_id, g in df.groupby("expr_id"):
        context_len = list(g["context_len"])[0]
        fig, ax = plot_df(g, x = "request_id", y = "throughput", group="engine_id", kind = "line", ylabel="Tokens / sec", ylim = (0, None))
        ax.set_title(f"Context len = {context_len}")
        pdf_pages.savefig(fig, bbox_inches="tight")
    ...
    for col in gpu_columns:
        tmp_df[col] /= 1000
        tmp_df.plot(x = "expr_id", y = col, marker = "o", ylabel = "GPU memory usage (GB)", ax=ax, grid = True, ylim = (0, None))
    pdf_pages.savefig(fig, bbox_inches="tight")
    pdf_pages.close()
